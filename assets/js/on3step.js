...
$(document).ready(function () {
  ...
  if ($Wcontainerpro.length) {
    // Initialize Isotope with filter for paket 1 ('.paket1')
    $Wcontainerpro.isotope({
      itemSelector: '.w-gallery',
      filter: '.paket1' // Only show elements of paket 1 initially
    });
    ...
    jQuery('.filt-projects-w').on('click', function (e) {
      ...
      if ($this.hasClass('selected')) {
        return false; // If already selected, do nothing
      }

      var $optionSetpro = $this.parents();
      $optionSetpro.find('.selected').removeClass('selected'); // Remove 'selected' class from previous
      $this.addClass('selected'); // Add 'selected' class to the current item

      var selector = $this.attr('data-project'); // Get the filter class from the clicked item
      $Wcontainerpro.isotope({ filter: selector }); // Apply filter

      return false;
    });

    // Layout Isotope once all images are loaded
    $Wcontainerpro.imagesLoaded().progress(function () {
      $Wcontainerpro.isotope('layout');
    });

    // Optional: Update the count of items displayed
